    ...
    public static InteractionResult placeWaypoint(Item item, Level level, Player player, UUID owner, BlockPos pos) {
        if (ModClient.CONFIG.items_place_landmarks && ModLists.items.contains(item)) {
            if (level.isClientSide()) {
                ...
                if (landmarks == null) {
                    HudUtils.sendOverlayMessage(player, Component.translatable("landmark.immersiveminimaps.disabled").withStyle(ChatFormatting.YELLOW));
                } else {
                    ...
                    if (landmarks.contains(owner, id)) {
                        ...
                        HudUtils.sendOverlayMessage(player, Component.translatable("landmark.immersiveminimaps.removed", pos.toShortString()).withStyle(ChatFormatting.GREEN));
                    } else {
                        landmarks.put(Landmark.create(owner, id, (builder) -> LandmarkComponentTypes.forBlock(builder, level, pos)));
                        HudUtils.sendOverlayMessage(player, Component.translatable("landmark.immersiveminimaps.added", pos.toShortString()).withStyle(ChatFormatting.GREEN));
                    }
                }
            }
